...
1. **Public Self-Service:** Container terpusat (`max-w-7xl`), header bertingkat dengan glassmorphism (`bg-white/92 backdrop-blur`), dan grid kartu layanan 1-3 kolom yang adaptif.
2. **Kiosk Touch Station:** Layar penuh terpusat (`max-w-5xl`), padding vertikal luas, tombol berorientasi sentuh satu sentuhan, dan layout tanpa scroll berlebih.
3. **TV Display Hall:** Grid 2 kolom berimbang (`h-screen overflow-hidden`) — panel pemanggilan loket aktif di sisi kiri dan video edukasi / antrian antrean berjalan di sisi kanan, dilengkapi running text marquee di bagian bawah.
...
### Shadow Vocabulary
- **Ambient Header:** `box-shadow: 0 18px 50px -32px rgba(14, 116, 144, 0.4)` — Menahan header mengapung lembut di atas kanvas publik.
- **Card Elevated:** `box-shadow: 0 24px 60px -48px rgba(15, 23, 42, 0.18)` — Memberi elevasi pada kartu statistik dan modul utama dashboard.
...
### Named Rules
**The Clean Surface Rule.** Seluruh kartu elevasi di atas background gradasi wajib memiliki background semi-transparan `bg-white/90` atau `bg-white` solid dengan border `border-cyan-100` atau `border-slate-200` untuk menjaga batas kontur.

## Shapes

- **Form Language:** Kurva membulat luas yang bersahabat dan ramah sentuhan.
- **Radius Scale:**
  - `rounded-2xl` (16px): Standar default untuk tombol, input text, dialog kecil, dan kartu statistik.
  - `rounded-3xl` (24px): Kartu layanan publik, container utama Kiosk, panel menu dropdown, dan footer section.
  - `rounded-xl` (12px): Sub-elemen internal, list item, dan badge ikon.
  - `rounded-full` (9999px): Badge status, navbar pill, dan tombol toggle ikon bulat.

## Components

### Buttons
- **Shape:** `rounded-2xl` (16px) dengan transisi halus (`transition duration-200`).
- **Primary:** Latar gradien Court Cyan (`from-cyan-600 to-blue-600` atau solid `#0e7490`), teks putih, shadow lembut `shadow-cyan-500/25`.
- **Kiosk Touch Button:** Tinggi minimal 64px (`h-16`), font tebal berukuran besar (`text-xl font-bold`), padding ekstra `px-8`.
- **Subtle / Ghost:** Latar `bg-cyan-50` dengan teks `text-cyan-900` dan border tipis `border-cyan-100`.

### Cards & Containers
- **Corner Style:** `rounded-2xl` hingga `rounded-3xl`.
- **Background:** `bg-white` atau `bg-white/90 backdrop-blur` dengan border `border-cyan-100/80` (publik) atau `border-zinc-200 dark:border-zinc-700` (internal).
...
### Inputs & Form Fields
- **Style:** Border tegas 2px (`border-slate-200`), latar putih, sudut `rounded-2xl`, tinggi minimal 48px (standar) atau 64px (kiosk).
...
### Badges & Chips
- **Style:** `rounded-full`, padding `px-3 py-1`, teks berukuran `text-xs font-semibold tracking-[0.18em] uppercase`.
- **Variants:** Emerald (Sukses / Jam Layanan), Amber (Menunggu), Red (Batal), Cyan (Layanan Aktif).
