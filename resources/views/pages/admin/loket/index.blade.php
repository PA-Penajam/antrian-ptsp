                ...
                <flux:table.rows>
                    @foreach ($counters as $counter)
                        <flux:table.row>
                            ...
                            <flux:table.cell>
                                <form method="POST" action="/admin/loket/{{ $counter->id }}" class="flex flex-col gap-2 xl:flex-row xl:items-start">
                                    @csrf
                                    @method('PUT')

                                    <flux:select name="queue_pool_id" size="sm" class="min-w-44">
                                        @foreach ($queuePools as $pool)
                                            <flux:select.option value="{{ $pool->id }}" :selected="$counter->queue_pool_id === $pool->id">
                                                {{ $pool->name }}
                                            </flux:select.option>
                                        @endforeach
                                    </flux:select>
