...
it('tv display reuses a persistent audio element for tts playback', function () {
    ...
    $response->assertOk()
        ...
        ->assertSee('debugEnabled', false)
        ->assertSee('TV Debug', false)
        ->assertSee('tts audio play failed', false)
        ->assertSee('pendingTtsText', false)
        ->assertSee('tts deferred until audio unlock', false)
        ->assertSee('tts replay after audio unlock', false)
        ->assertSee('clearAudioSource(audio)', false)
        ->assertSee('isSamsungTv', false)
        ->assertSee('isLgTv', false)
        ->assertSee('shouldIsolateTtsAudio', false)
        ->assertSee('get shouldIsolateTtsAudio() { return false }', false)
        ->assertSee('audio.volume = this.isLgTv ? 0 : this.ttsVolume', false)
