...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Daftar Antrian' / 'Ambil Antrian' link to open the booking page (use the visible link on the homepage).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/header/div/nav/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the first service card ('Layanan E2E') to select a service and proceed to the booking form (click element index 1505).
        frame = context.pages[-1]
        # Click element
